...
def inference(model_name, weight_file, image_size, image_list, data_dir, label_dir, return_results=True, save_dir=None,
    ...
    for img_num in image_list:
        ...
        resize_ratio = min(float(image_size[0])/image.shape[0],float(image_size[1])/image.shape[1])
        ...
        print (result_img.palette)# = label.palette
        ...
        if save_dir:
            ...
            image_bak.save(os.path.join(save_dir, img_num + '_test.png'))
    return results
...
if __name__ == '__main__':
    ...
    results = inference(model_name, weight_file, image_size, image_list, data_dir, label_dir,return_results=return_results,save_dir=save_dir)
